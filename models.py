...
class conv_block(nn.Module):
    def __init__(self, in_channels, out_channels, k, s, p, act=None, upsample=False, spec_norm=False):
        super(conv_block, self).__init__()
        # TransposeConv
        if upsample:
            if spec_norm:
                self.ConvT = spectral_norm(torch.nn.ConvTranspose2d(in_channels, out_channels, kernel_size=k, stride=s, padding=p, bias=True))
            else:
                self.ConvT = torch.nn.ConvTranspose2d(in_channels, out_channels, kernel_size=k, stride=s, padding=p, bias=True)
        else:
            if spec_norm:
                self.ConvT = spectral_norm(torch.nn.Conv2d(in_channels, out_channels, kernel_size=k, stride=s, padding=p, bias=True))
            else:
                self.ConvT = torch.nn.Conv2d(in_channels, out_channels, kernel_size=k, stride=s, padding=p, bias=True)
        # Activate
        if "leaky" in act:
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.upsample_1 = conv_block(16, 16, 4, 2, 1, act="leaky", upsample=True)
        self.upsample_2 = conv_block(16, 16, 4, 2, 1, act="leaky", upsample=True)
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.cmp_1 = CMP(in_channels=16)
        self.downsample_1 = conv_block(16, 16, 3, 2, 1, act="leaky")
        self.cmp_2 = CMP(in_channels=16)
        self.downsample_2 = conv_block(16, 16, 3, 2, 1, act="leaky")
        self.decoder = nn.Sequential(
